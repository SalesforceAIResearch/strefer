...
def finetune_one_epoch(
    ...
    # for step_num, samples in enumerate(dataset.dataloader):
        ...
        if ((step_num + 1) % args.checkpoint_steps == 0):
            save_checkpoint_step(model, optimizer, lr_scheduler, epoch, step_num, args)
...
def finetune_one_epoch_multidatasets(
    args,
    model,
    epoch,
    datasets: [DataInfo],
    ...
    """
    # calculate the number of steps in an epoch
    num_batches_per_epoch = len(datasets[0].dataloader)
    total_training_steps = num_batches_per_epoch * args.num_epochs
    ...
    for step_num, batches in tqdm(
        ...
        for dataset_ix, samples in enumerate(batches):
            # images, (input_ids, attention_mask) = samples
            # unpack the batch and move to device
            images = samples["images"]
            ...
            sample_collection_path = os.path.join(
                args.run_name, 'dataset{}_rank{}.pkl'.format(dataset_ix, args.rank))
            
            if os.path.exists(sample_collection_path) and os.path.getsize(sample_collection_path):
                with open(sample_collection_path, 'rb') as f:
                    sample_collection = pickle.load(f)
            else:
                sample_collection = []
            sample_collection += sample_id
            with open(sample_collection_path, 'wb') as f:
                pickle.dump(sample_collection, f)  
            ###############
            ...
            mask_frames = samples["mask_frames"]
            frame_nums = samples["frame_nums"]
            masks = samples["masks"]
            ann_indices = samples["ann_indices"]
...
def get_autocast(precision, cache_enabled=True):
    ...
    elif precision in ["amp_bfloat16", "amp_bf16", "bf16"]:
...
def load_checkpoint(args, model, pretrained=False):
    ...
    if 'vision_tokenizer.latents' in msd.keys():
        ...
        if msd_current['vision_tokenizer.latents'].shape != msd['vision_tokenizer.latents'].shape:
            msd["vision_tokenizer.latents"] = msd_current['vision_tokenizer.latents']

    # Get the existing weights from checkpoint
    weight_key = 'lang_model.model.embed_tokens.additional_embedding.weight'
    if msd_current[weight_key].shape[0] > msd[weight_key].shape[0]:
        additional_embedding_weights = msd[weight_key]
        new_weights = msd_current[weight_key]
        new_weights[:additional_embedding_weights.shape[0]] = additional_embedding_weights  # Copy old weights
        msd[weight_key] = new_weights
    elif msd_current[weight_key].shape[0] < msd[weight_key].shape[0]:  # Old model checkpoint had more embeddings
        additional_embedding_weights = msd[weight_key]
        msd[weight_key] = additional_embedding_weights[:msd_current[weight_key].shape[0]]  # Trim extra weights

    weight_key = 'lang_model.lm_head.additional_fc.weight'
    if msd_current[weight_key].shape[0] > msd[weight_key].shape[0]:
        additional_embedding_weights = msd[weight_key]
        new_weights = msd_current[weight_key]
        new_weights[:additional_embedding_weights.shape[0]] = additional_embedding_weights  # Copy old weights
    ...
    weight_key = 'lang_model.lm_head.additional_fc.bias'
    if msd_current[weight_key].shape[0] > msd[weight_key].shape[0]:
        additional_embedding_weights = msd[weight_key]
        new_weights = msd_current[weight_key]
        new_weights[:additional_embedding_weights.shape[0]] = additional_embedding_weights  # Copy old weights
...
def save_checkpoint_step(model, optimizer, lr_scheduler, epoch, step, args):
    """
    Save training checkpoint with model, optimizer, and lr_scheduler state.
    """
    if args.fsdp:
    ...
    if args.rank == 0:
        ...
        print(f"Saving checkpoint to {args.run_name}/checkpoint_{epoch}_{step}.pt")
        torch.save(checkpoint_dict, f"{args.run_name}/checkpoint_{epoch}_{step}.pt")
        if args.report_to_wandb and args.save_checkpoints_to_wandb:
            wandb.save(f"{args.run_name}/checkpoint_{epoch}_{step}.pt")
