...
from open_flamingo.src.kosmos import KosmosInstruct, Kosmos
from open_flamingo.src.llava import Llava
...
class SupervisedPrediction(Loss):
    ...
    def __call__(
        ...
        # call forward
        with autocast():
            if isinstance(model.module, KosmosInstruct) or \
               isinstance(model.module, Kosmos) or \
                isinstance(model.module, Llava):
                loss = model(
                    vision_x=images,
