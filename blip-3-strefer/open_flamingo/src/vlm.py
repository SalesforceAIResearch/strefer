...
from .helpers import DecoupledEmbedding, DecoupledLinear, VLMOutputWithPast, LinearPatchProjection, PerceiverResampler
from transformers.modeling_outputs import CausalLMOutputWithPast
from transformers import CLIPVisionModel
from transformers.models.siglip.modeling_siglip import SiglipVisionTransformer, SiglipVisionModel
...
class VLMWithLanguageStream(VLM):
    ...
    def _prepare_inputs_for_forward(
        ...
        for i in range(B):
            ...
            region_token_nums_this_sample = region_token_nums[i]
            if region_token_nums_this_sample != [0]:
                region_token_idxs = torch.where(lang_x[i] == self.region_token_id)[0]
            ...
            for img_num in range(len(image_token_idxs)):
                ...
                # Offset the rest of region tokens with current num_vis_tokens
                if region_token_nums_this_sample != [0]:
                    for j in range(len(region_token_idxs)):
                        region_token_idxs[j] += (num_vis_tokens - 1)
            ...
            if region_token_nums_this_sample != [0]:        
                ...
                for region_num in range(len(region_token_idxs)):
                    reg_idx = region_token_idxs[region_num]
                    
                    num_reg_tokens = region_token_nums_this_sample[region_num]
                    reg_attention_mask = torch.ones(
                        num_reg_tokens, dtype=torch.long
                    ).to(attention_mask.device)
                    
                    # Offset the rest of region tokens with current num_reg_tokens
                    for j in range(region_num+1, len(region_token_idxs)):
                        region_token_idxs[j] += (num_reg_tokens - 1)
    
            
                    new_embed = torch.cat(
                        (
                            new_embed[:reg_idx],
                            region_tokens[reg_tokens_index:reg_tokens_index+num_reg_tokens],
                            new_embed[reg_idx + 1 :],
                        ),
                        dim=0,
                    )
                    
                    reg_tokens_index += num_reg_tokens
                    new_attention_mask = torch.cat(
                        (
                            new_attention_mask[:reg_idx],
                            reg_attention_mask,
                            new_attention_mask[reg_idx + 1 :],
                        ),
                        dim=0,
                    )
                    if has_labels:
                        new_label = torch.cat(
                            (
                                new_label[:reg_idx],
                                torch.ones(num_reg_tokens, dtype=torch.long).to(
                                    labels.device
                                )
                                * -100,
                                new_label[reg_idx + 1 :],
