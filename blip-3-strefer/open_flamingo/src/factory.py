...
from .kosmos import Kosmos, KosmosInstruct
...
MODEL_FAMILY_TO_CLASS = {
    ...
    "blip": BLIP,
    "llava": Llava,
    "kosmos-instruct": KosmosInstruct,
}
...
def create_model_and_transforms(
    ...
    **model_kwargs,
    ...
    # decide on the dtype
    if 'use_flash_attention_2' in model_kwargs and model_kwargs['use_flash_attention_2'] == True:
        attn_implementation = "flash_attention_2"
        lang_dtype = torch.bfloat16
    else:
        attn_implementation = None
        lang_dtype = torch.float32
    
    if 'use_flash_attention_2' in model_kwargs:
        model_kwargs.pop('use_flash_attention_2')
    unfreeze_vision_encoder = model_kwargs.pop('unfreeze_vision_encoder', False)
    ...
    lang_model = AutoModelForCausalLM.from_pretrained(
        ...
        torch_dtype=lang_dtype,
    )
    if 'llama' in lang_model_path.lower() and added_pad_token:
        print("Adding <pad> token to the language model and rezising the token embeddings to match the tokenizer. It will take a while....")
        lang_model.resize_token_embeddings(len(text_tokenizer))
        num_new_tokens = 1
        input_embeddings = lang_model.get_input_embeddings().weight.data
        output_embeddings = lang_model.get_output_embeddings().weight.data
    ...
    model.lang_model.config.vocab_size = len(text_tokenizer)  # 32015 -> 32017
