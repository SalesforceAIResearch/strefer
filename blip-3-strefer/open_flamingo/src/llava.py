...
class Llava(VLMWithLanguageStream):
    def __init__(
        ...
        pad_token_id: int,
        mm_projector_type: str = 'mlp2x_gelu', # Same as Llava-1.5.
        ...
        elif vision_encoder.__class__.__name__ == 'CLIPVisionModel':
            grid_size_base = vision_encoder.config.image_size // vision_encoder.config.patch_size
        ...
        super().__init__(
            ...
            vision_tokenizer=LinearPatchProjection(dim_visual=vis_feature_dim,
                                                    ...
                                                    num_patches=(grid_size[0] * grid_size[1]) + 1), # OpenAI's CLIP has 1 more output token.
        ...
        if self.image_aspect_ratio == 'anyres':
            embed_std = 1 / torch.sqrt(torch.tensor(vis_feature_dim, dtype=self.lang_model.dtype))
            self.image_newline = nn.Parameter(
                torch.randn(vis_feature_dim, dtype=self.lang_model.dtype) * embed_std
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if vision_x is not None:
            if self.image_aspect_ratio in ['anyres', 'anyres-legacy']:
        ...
        new_inputs = self._prepare_inputs_for_forward(
            ...
            vision_attention_mask=vision_attn_masks,
        ...
        **kwargs,
        ...
        if vision_x is not None:
            if self.image_aspect_ratio in ['anyres', 'anyres-legacy']:
                ...
                vision_attn_masks = None
            # Prepare text embeds for instruction-aware image query sampling.
            text_embeds = self.lang_model.get_input_embeddings()(lang_x)
            vision_tokens = self.vision_tokenizer(vision_features)
        else:
            vision_tokens = None
